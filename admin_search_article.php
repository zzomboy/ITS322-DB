	...
	<div class="admin_right">
		<div class="admin_tool_box">
			...
			<select class="sortby_tool" onchange="location = this.value;">
				...
				<option value="?sortby=article_time desc&searchword=<?php echo $_GET['searchword']; ?>">Newest - Oldest</option>
				<option value="?sortby=article_time&searchword=<?php echo $_GET['searchword']; ?>">Oldest - Newest</option>
				<option value="?sortby=article_name&searchword=<?php echo $_GET['searchword']; ?>">Name A - Z</option>
				<option value="?sortby=article_name desc&searchword=<?php echo $_GET['searchword']; ?>">Name Z - A</option>
				<option value="?sortby=tag_name&searchword=<?php echo $_GET['searchword']; ?>">Tag A - Z</option>
				<option value="?sortby=tag_name desc&searchword=<?php echo $_GET['searchword']; ?>">Tag Z - A</option>	
			</select>
     	</div>
		<div class="clear"></div>

		<div class="table_heading">
    		<h2>Searching acrticle for "<?php echo $_GET['searchword']; ?>"</h2>
    	</div>
    	<div class="clear"></div>

		<table class='article_tb'>
<?php
		$q = "SELECT a.*,t.tag_name FROM tags t,article a WHERE t.tag_id=a.tag_id and (a.article_name like '%".$_GET['searchword']."%' or t.tag_name like '%".$_GET['searchword']."%') ORDER BY ".$sortby;
